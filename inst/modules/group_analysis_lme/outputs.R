...
plot_cluster_means <- function(collapsed_data, electrode_labels) {

    electrode_labels %?<-% local_data$electrode_labels
    collapsed_data %?<-% local_data$collapsed_data
    
    shiny::validate(shiny::need(!is.null(electrode_labels), message = 'Clustering not run yet...'))
  
    
    electrode_labels$subject
    electrode_labels$electrode
    electrode_labels$cluster
    # collapsed_data$cluster = ...
    
    
    mean_data = aggregate(Power ~ Time + Group + cluster, mean, data=collapsed_data)
    
    # draw plots, one panel per cluser, one line per group
    
    
    mean_data %>% split((.)$cluster) %>% lapply(do_plot_cluster_mean)
    
}

do_plot_cluster_mean <- function(data) {
