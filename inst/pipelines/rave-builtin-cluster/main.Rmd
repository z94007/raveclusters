...
local({
  f <- c(
    ...
    file.path(rstudioapi::getActiveProject(), "inst", 'pipelines', 'rave-builtin-cluster', 'settings.yaml')
  )
  f <- f[file.exists(f)][[1]]
...
tmp <- ravetools::baseline_array(collapsed_array, 2L, baseline_indexpoints = which(column_sel), method = "zscore")
...
dis <- raveclusters::calculate_distance(indata_analysis, method =distance_method)
```
...
if(ncol(indata_analysis) == 2) {
  ...
  mds_result <- cmdscale(as.dist(
    raveclusters::calculate_distance(indata_analysis, method = mds_distance)
  ),
  k = 2)
...
local({
  pipe_dir <- raveio:::activate_pipeline(file.path(rstudioapi::getActiveProject(), "inst", 'pipelines', 'rave-builtin-cluster'))
  targets::tar_destroy(ask = FALSE)
  targets::tar_visnetwork(targets_only = TRUE, shortcut = FALSE, level_separation = 200, zoom_speed = 0.3)
})
```
