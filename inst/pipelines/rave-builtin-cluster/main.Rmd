...
* For each group, slice the columns (`baseline` list items) such that the corresponding time points fall within the `time_window`
...
indata_analysis <- lapply(baseline, function(item){
  # item <- baseline[[1]]
  item$data[, item$time %within% time_window, drop = TRUE]
})
...
mse <- lapply(cluster_idx, function(ci){
  apply(
    indata_plot[clusters$Cluster == ci,,drop = FALSE], 2, function(x){
      dipsaus::mean_se(x, na.rm = TRUE, se_na_as_zero = TRUE)
    })
})
# mse <- lapply(cluster_idx, function(ci){
#   ### Question: collapsed data or baselined?
#   baseline
