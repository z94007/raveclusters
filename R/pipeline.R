...
ravecluster <- function(names, ..., 
                        ...
                        .verbose_level = c("silent", "verbose", "summary", "timestamp"),
  ...
  re <- raveio::pipeline_run(pipe_dir = pipeline_path, scheduler = .scheduler, type = .type, envir = new.env(parent = globalenv()), names = names, async = FALSE, reporter = .verbose_level)
  
  if(.values) {
    re <- re$get_values(names = names)
  }
  re
