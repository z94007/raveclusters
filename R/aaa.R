...
dev_raveclusters <- function(expose_functions = FALSE, reload = TRUE){
  ...
  .fs = c(.fs, system.file('tools/input_widgets.R', package = 'raveclusters'))
...
debug_module <- function(module_id, interactive = FALSE, check_dependencies = TRUE, force_update_remote = FALSE){
  ...
  env = dev_raveclusters(expose_functions = F, reload = TRUE)
  ...
  if(!has_subject){
    cat2('Warning: No subject found! Loading demo subject', level = 'WARNING')
    env$mount_demo_subject()
  }
